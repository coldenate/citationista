...
function CitationFinderWidget() {
    ...
    /** floating-widget context (needed for key-steal + move-with-caret) */
    const ctx = useRunAsync(
        async () => await plugin.widget.getWidgetContext<WidgetLocation.FloatingWidget>(),
        []
    );

    /** “citation” (default) vs “bib” mode set by the invoking command */
    ...
    /* ------------------------------------------------------------------
     * Incremental search: every time `query` changes we ask RemNote’s
     * full-text engine for up to 50 hits, then filter to Zotero items.
     * ------------------------------------------------------------------ */
    ...
    const results =
        useTracker(
            async (rp) => {
                ...
                // Build a rich-text token for search
